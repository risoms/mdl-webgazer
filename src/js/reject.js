function browserHover(){
	$(".imgContainer").hover(
		function (){
			$(this).css("background-color", 'rgba(78, 141, 245, 0.5)');
		},
		function (){
			$(this).css("background-color", 'rgba(78, 141, 245, 0)');
		}
	);
}
...
function checkBrowser(){
	...
	browser=='Firefox'||browser=='firefox'||browser=='Edge'||browser=='MSIE'||browser=='IEMobile'){
		console.log('%c0.browser incompatiable...','color: orange');
		remove_container = $('.container')
		remove_container.remove()
		document.write('<style>.imgContainer{float:left; width: calc(100%/2);}</style>');
			...
			+'<div class="container">'
				+'<div class="container-inner">'
					+'<a href="'+url+'">'
						+'<p>'+url+'</p>'
					+'</a>'
					+'<p>'+browser+' will not be able to run this experiment.</p>'
					+'<p>Instead, please save or bookmark the the link above and run the experiment on one of the following browsers.</p>'
					+'<div class="image123">'
						+'<div class="imgContainer" id="chrome">'
							+'<a href="https://www.google.com/chrome/browser/desktop/">'
								+'<img src="dist/img/chrome.png" height="150" width="150"/>'
							+'</a>'
							+'<p>Chrome (preferred) </p>'
						+'</div>'
						+'<div class="imgContainer" id="opera">'
							+'<a href="http://www.opera.com/download">'
								+'<img src="dist/img/opera.png" height="150" width="150"/>'
							+'</a>'
							+'<p>Opera</p>'
						+'</div>'
						+'<p>To download, simply click on the logo of the browser.</p>'
					+'</div>'
				+'</div>'
			+'</div>'
		);
