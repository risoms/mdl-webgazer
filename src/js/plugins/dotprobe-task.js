	...
	plugin.trial = function (display_element, trial) {
		...
		var end_trial = function () {
			...
			var trial_data = {
				...
				'lum': '', //relative luminance (out of 255)
			...
			//-------collect inattention parameters
			//isFullscreen updated within is_Fullscreen function
			trial_data['isFullscreen'] = is_Fullscreen();
			//check if tab is focused
			trial_data['isPageVisible'] = is_PageVisible();
			//luminance
			trial_data['lum'] = getImageBrightness();
			//window size
			trial_data['windowSize.px'] = [window.innerWidth+"x"+window.innerHeight];

			//console
			if (debug){
				console.log('%cisFullscreen: %s;','color: orange',trial_data['isFullscreen']);
				console.log('%cisPageVisible: %s;','color: orange',trial_data['isPageVisible']);
				console.log('%cgetImageBrightness: %d;','color: orange',trial_data['lum']);
			};
