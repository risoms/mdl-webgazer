...
//loadingbar
function loadingbar(){
	loading = new ProgressBar.Circle('#progress-container', {
		color: '#ffffff',
...
//-------------------------------------------------------------------Check is pagevisible
//check if user switched tabs in browser window
var is_PageVisible = (function () {
...
function is_Fullscreen() {
	if ((window.innerHeight < (.85*window.screen.height)) || (window.innerWidth < (.85*window.screen.width))) {
		...
		//hide canvas
		$("#jspsych-stim-canvas").hide()
		//set fullscreen false for trial
		isFullscreen = false;
		$notFullscreen = $("<div style=''>"+
...
function isBreak(){
	...
	$('#jspsych-fullscreen-btn').on('click', function() { 
		...
		$($Break).remove();
		//show canvas
		$("#jspsych-stim-canvas").show();
		//resume webgazer after break
		if (subject.webcam_used == true){webgazer.resume()}; 
		//resume experiment
...
function getParticipantId() {
	...
	if (debug){
		sub_idc = sub_idc || {uname: jsPsych.randomization.randomID().substring(0,6)};
	};
	return sub_idc;
};

/**
 * control visibility of webgazer canvas elements (face, box). Only used with debug
 *
 * @param {boolean} debug
 */
function control_canvas_visibility(debug){
	//show box and video
	if (debug){
		console.log('%ccontrol_canvas_visibility() debug','color: green');
		$("#webgazerVideoFeed").css({"visibility":'visible'});
		webgazer.showFaceOverlay(true)
			.showFaceFeedbackBox(true)
			.showPredictionPoints(true);
		//create button to show/hide video
		$('<div class="settings"></div>').appendTo('body')
		.css({"width":'100px',"height":'100px',"position":'absolute',
		"left":"calc(100vw - 100px - 30px)","top":"calc(100vh - 100px - 30px)",
		"content":"url(dist/img/settings.svg"})
		.attr('is_display','true')
		.on('click', function() {
			//if hidden
			if ($(this).attr('is_display') === 'false'){
				console.log('on');
				//update visibilty
				$("#webgazerVideoFeed").css({"visibility":'visible'});
				$("#webgazerFaceFeedbackBox").css({"display":'block'});
				$("#webgazerFaceOverlay").css({"display":'block'});
				$("#webgazerGazeDot").css({"display":'block'});
				//save attribute
				$(this).attr('is_display', 'true');
			//if visible
			} else {
				console.log('off')
				//update visibilty
				$("#webgazerVideoFeed").css({"visibility":'hidden'});
				$("#webgazerFaceFeedbackBox").css({"display":'none'});
				$("#webgazerFaceOverlay").css({"display":'none'});
				$("#webgazerGazeDot").css({"display":'none'});
				//save attribute
				$(this).attr('is_display', 'false');
			}
		});
	}
};
...
function postDatabase() {
	...
	.done(function (response) {
		...
		//console.log(response);
		if (pid.subsession=='abc'){
			setTimeout(redirectREDCap, 10000,response);//added setTimeout as redirect back to REDCap
		}
	})
	.fail(function (response) {
...
//see:http://www.sona-systems.com/help/qualtrics.aspx
function redirectSona() {
	id_redirect = '&survey_code=' + participant_id
	$(location).attr('href', 'https://utexas.sona-systems.com/webstudy_credit.aspx?experiment_id=671&credit_token=a3cace52b70c45cdbd8d6b64b44c5769' + id_redirect)
};

//redirect back to SONA
function redirectREDCap(redirect_url) {
...
//save calibration data and upload to server
function save_calibration(){
	console.log('%csave calibration data','color: green');
    //convert raw data to csv readable
    var data = _.map(mdl.calibration, function(d){
        return JSON.stringify(_.values(d));
    })
    .join('\n') 
    .replace(/(^\[)|(\]$)/mg, '');

    //filename
    var filename = `${participant_id}_${pid.session}_${calibration_block}_calibration.csv`
    //create csv content from MIME, headers, and raw data
    var csv = [JSON.stringify(Object.keys(mdl.calibration[0]))].join('\n').replace(/(^\[)|(\]$)/mg, '') + '\n' + data
	...
	$.ajax({
		...
		if(debug){
			console.log('calibration uploaded: '+response);
		};
		//clear memory
		mdl.calibration = [];
		...
		if(debug){
			console.log('Fail! calibration not uploaded: '+response);
		};
	});
};
...
function saveData(filename, filedata) {
	...
	if (ltask_acc.length==0){
		...
		pid.score = (_.sum(ltask_acc)/_.mean(lrt))*1000;
	};
	pid.webcam = subject.webcam_used;
		...
		if (jsPsych.data.getTrialsOfType("dotprobe-task").length == task.length){
			...
			if (debug){
				document.write(
					...
					'<p>Please do not close your browser until you have been redirected back to your survey.<br>'+
					'to ensure that you receive credit for participating.</p>'
				)
			} else {
				document.write(
					'<script src="dist/js/jquery-3.1.0.min.js"></script>' +
	...
	.fail(function(jqXHR, textStatus, error, response){
		...
		filename = participant_id + "_" + pid.session + pid.subsession + ".csv"
		document.write(
			'<script src="dist/js/jquery-3.1.0.min.js"></script>' +
			'<link href="dist/css/jspsych.css" rel="stylesheet" type="text/css">' +
			'<link href="dist/css/experiment.css" rel="stylesheet" type="text/css">' +
			...
			'<p>Please download the following <code id="filename">'+filename+'</code> and email to rochelstewart@utexas.edu.</p>'
		);
		//save local
		if (debug) {
			postDatabase();//post participation to database
			$("#filename").click(function(){saveLocalCSV(filedata, filename, 'csv')});
			$("#filename").hover(function(){$('#filename').css({'background-color':'#d19017'})});
			$("#filename").mouseout(function(){$('#filename').css({'background-color':'#666'})});
		} else {
			postDatabase();//post participation to database
			$("#filename").click(function(){saveLocalCSV(filedata, filename, 'csv')});
			$("#filename").hover(function(){$('#filename').css({'background-color':'#d19017'})});
