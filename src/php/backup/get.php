<?php
/*
 * get data from database before begining experiment
 * php version: 5.6.25
 */
...
/*check if attempting second session today*/
if ($day0 == $day1){
	//wait until 2am tomorrow
	$date2 = date("Y-m-d H:i:s", (strtotime($day0) + 93600));
	$wait_until = array(array("start_date" => $date0,"wait_date" => $date2, "ready" => "wait", "condition" => $cndt['group']));
	echo json_encode($wait_until);
} else {
	//check if later than 2am next day
	if(date("Hi") < "200") {
		//wait until 2am today
		$date2 = date("Y-m-d H:i:s", (strtotime($day1) + 7200));
		$wait_until = array(array("start_date" => $date0,"wait_date" => $date2, "ready" => "wait", "condition" => $cndt['group']));
		//echo ('wait until after 2am');
		echo json_encode($wait_until);
	} else {
